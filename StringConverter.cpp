...
namespace StringConverter {
    ...
    std::u8string StringConverter::ConvertUTF8(const char* data, int length) {
        return std::u8string(reinterpret_cast<const char8_t*>(data), length );     
    }
    ...
    std::vector<std::vector<char32_t>> StringConverter::GetUTF32WordsFromUTF8String(const std::u8string& wyrazenie) {
        if (wyrazenie.empty()) {
